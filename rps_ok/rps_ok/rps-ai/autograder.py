...
def test_Agents():
    ...
    if choice == "4":
        ...
        """
        Trying to test against a smart strategy
        def smart_strategy(strategy):
            rock_freq, paper_freq, scissors_freq = history.get_opponent_frequency(strategy)
            expected_strategy = agents.triple_biased_strategy(1-rock_freq, 1-paper_freq, 1-scissors_freq)
            return agents.counter(expected_strategy)
        for i in range(2):
            rates = np.random.random(3)
            rates /= rates.sum()
            strategy1 = agents.triple_biased_strategy(rates[0], rates[1], rates[2])
